...
# save the results
import csv
with open('results_isometric_total.csv', "w") as csvfile:
    resultswriter = csv.writer(csvfile, dialect='excel')
    for i in range(0, len(time)):
        results_row = [time[i], SL[i], afterload[i], Ca_i[i], active_tension[i],  F_total[i], XB_cycling[i], gxbT[i], XBpostr[i], hfT[i], SOVFThick[i], xXBpostr[i], XBprer[i], xXBprer[i], fxbT[i], hbT[i], gappT[i], fappT[i], P[i], N[i], kn_pT[i], kp_nT[i], t_interval[i], iteration_time[i], tstart[i]]
